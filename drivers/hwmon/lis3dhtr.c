...
static void lis3dh_acc_device_power_off(struct lis3dh_acc_data *acc)
{
	...
	if (acc->hw_initialized) {
		printk("------lukai------(acc->hw_initialized)\n");
		#if 0
		if(acc->pdata->gpio_int1){
			printk("------lukai------disable_irq_nosync(acc->irq1);\n");
			disable_irq_nosync(acc->irq1);
			}
		if(acc->pdata->gpio_int2){			
			printk("------lukai------disable_irq_nosync(acc->irq2);\n");			
			disable_irq_nosync(acc->irq2);
			}
		#endif
		acc->hw_initialized = 0;
		...
		u8 reg_address, u8 new_value)
{
	...
	//Sets configuration register at reg_address NOTE: this is a straight overwrite
		buf[0] = reg_address;
		buf[1] = new_value;
		err = lis3dh_acc_i2c_write(acc, buf, 1);
		if (err < 0)
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
	...
	if (!acc->input_dev) {
		...
		printk(SENSOR_TAG "input device allocation successed\n");
	}
	...
	if (err) {
		...
		printk(SENSOR_TAG "register input device %s\n",acc->input_dev->name);
	}
...
{
	struct lis3dh_acc_data *kd =
		container_of(h, struct lis3dh_acc_data, early_suspend);
	struct i2c_client *client = kd->client;
	struct lis3dh_acc_data *tik = i2c_get_clientdata(client);

	if (tik->on_before_suspend)
		lis3dh_acc_enable(tik);
...
static void lis3dh_early_suspend(struct early_suspend *h)
{
	...
	tik->on_before_suspend = atomic_read(&tik->enabled);
	lis3dh_acc_disable(tik);
		...
		const struct i2c_device_id *id)
{
	...
	printk(SENSOR_TAG "%s: probe start.\n", LIS3DH_ACC_DEV_NAME);
	...
	*/
	...
	if (err < 0) {
		...
		printk(SENSOR_TAG "power on success\n");
	}
	...
	if (err < 0) {
		...
		printk(SENSOR_TAG "input init successed\n");
	}
	...
	acc->early_suspend.level   = EARLY_SUSPEND_LEVEL_DISABLE_FB - 10;
        register_early_suspend(&acc->early_suspend);
#endif

	printk(SENSOR_TAG "probed success \n");
