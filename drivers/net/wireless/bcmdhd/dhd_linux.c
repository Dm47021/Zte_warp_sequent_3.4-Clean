...
dhd_concurrent_fw(dhd_pub_t *dhd)
{
	...
	if ((!op_mode) && (strstr(fw_path, "_p2p") == NULL) &&
		(strstr(fw_path, "_apsta") == NULL) && (strstr(fw_path, "test") == NULL)) {
...
//Hector 20121016 add for white list+++
#define 	white_command 	20
...
int wl_cfg80211_add_white_list(struct net_device *net, char *command, int num)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(net);
	int i = 0, ret = 0, length, mac_mode = 2;
	int white_list_num = (num - white_command) / sizeof(struct mac_acl_entry);
        
	struct mac_acl_entry * white_list = (struct mac_acl_entry *)(command + white_command);
	struct mflist *white_maclist;
	white_maclist = (struct mflist *)(kmalloc(sizeof(struct mflist), GFP_ATOMIC));

	printk("shaohua command length = %d, white_list num = %d\n", num, white_list_num);

	if(white_list_num > 16) {
		printk("dhd only allow 16 white_list \n");
		white_list_num = 16;
	}

	for(i = 0; i < white_list_num; i++)
		printk("hostapd send us ccept mac=%02x:%02x:%02x:%02x:%02x:%02x \n",
                        white_list[i].addr[0],white_list[i].addr[1],white_list[i].addr[2],
                        white_list[i].addr[3],white_list[i].addr[4],white_list[i].addr[5]);
	
	//set macmode = 2
	ret = dhd_wl_ioctl_cmd(&(dhd->pub), WLC_SET_MACMODE, (char *)&mac_mode, 4 ,TRUE, 0);
	printk("shaohua test %s, ret = %d \n", __func__, ret);

	//add maclist to white_maclist
	white_maclist->count = white_list_num;
	for(i = 0; i < white_list_num; i++) {	
		memcpy(white_maclist->ea[i].octet, white_list[i].addr, 6);
	}

	//set maclist to firmware
	length = sizeof(white_maclist->count) + white_maclist->count * sizeof(struct ether_addr);
	ret = dhd_wl_ioctl_cmd(&(dhd->pub), WLC_SET_MACLIST, white_maclist, length, true, 0);
	
	printk("shaohua exit %s, ret = %d \n", __func__, ret);
...
//Hector 20121024 add for WMM+++
#if defined(WMM_PS)
	/*enable wmm-ps mode to auto */
	bcm_mkiovar("wme_qosinfo", (char *)&qosinfo, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
	
	bcm_mkiovar("wme_auto_trigger", (char *)&auto_trigger, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
	//dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif  // WMM_PS
//Hector 20121024 add for WMM---
