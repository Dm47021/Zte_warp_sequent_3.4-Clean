/* drivers/i2c/chips/akm8962.c - akm8962 compass driver
 *
...
#define AKM8962_DEBUG                1
#define AKM8962_DEBUG_MSG        1
#define AKM8962_DEBUG_FUNC        0
#define AKM8962_DEBUG_DATA        0
#define MAX_FAILURE_COUNT        3
#define AKM8962_RETRY_COUNT        10
#define AKM8962_DEFAULT_DELAY        100

#if AKM8962_DEBUG_MSG
#define AKMDBG(format, ...)        printk(KERN_INFO "AKM8962 " format "\n", ## __VA_ARGS__)
#else
#define AKMDBG(format, ...)
#endif

#define ZTE_TAG         "[FYA@MSENSOR]"

#if AKM8962_DEBUG_FUNC
#define AKMFUNC(func) printk(KERN_INFO "AKM8962 " func " is called\n")
#else
#define AKMFUNC(func)
#endif
...
static short akmd_delay = AKM8962_DEFAULT_DELAY;
...
static int AKECS_GetData(char *rbuf, int size)
{
        ...
        if (!atomic_read(&data_ready)) {
                ...
                if (!atomic_read(&suspend_flag)) {
                        ...
                        if (failure_count >= MAX_FAILURE_COUNT) {
                                printk(KERN_ERR
                                       "AKM8962 AKECS_GetData: successive %d failure.\n",
                                       failure_count);
        ...
        rbuf[0] = AK8962_REG_ST1;
        return AKI2C_RxData(rbuf, SENSOR_DATA_SIZE);
...
static int AKECS_GetOpenStatus(void)
{
        ...
        if(atomic_read(&open_flag) == 1)
                AKMDBG("call in %s ,open_flag = 1",__func__);
...
static int AKECS_GetCloseStatus(void)
{
        wait_event_interruptible(open_wq, (atomic_read(&open_flag) <= 0));
        if(atomic_read(&open_flag) == 1){
                AKMDBG("[FYA]call in %s ,open_flag = 1",__func__);
                AKMDBG("[FYA]call in %s ,wait_event_interruptible(open_wq, (atomic_read(&open_flag) <= 0)) is hanging",__func__);
        }
...
static int akm_aot_release(struct inode *inode, struct file *file)
{
        ...
        AKMDBG("[FYA]call in %s ,open_flag was set to %d (0?)",__func__,atomic_read(&open_flag));
                          ...
                          unsigned int cmd, unsigned long arg)
{
        ...
        switch (cmd) {
                ...
                case ECS_IOCTL_APP_SET_MVFLAG:
                        ...
                        break;
                case ECS_IOCTL_APP_SET_DELAY: //set delay time
                        if (copy_from_user(&flag, argp, sizeof(flag))) {
                                return -EFAULT;
                        }
                        break;
...
{
        printk(ZTE_TAG "akm8962_early_suspend");
        ...
        atomic_set(&open_flag, 0);
        AKMDBG("[FYA]call in %s ,open_flag was set to %d",__func__,atomic_read(&open_flag));
        wake_up(&open_wq);
        //disable_irq(this_client->irq);
        printk(ZTE_TAG "suspended with flag = %d", 
               atomic_read(&reserve_open_flag));
}

static void akm8962_early_resume(struct early_suspend *handler)
{
        printk(ZTE_TAG "akm8962_early_resume");
        //enable_irq(this_client->irq);
        atomic_set(&suspend_flag, 0);
        atomic_set(&open_flag, atomic_read(&reserve_open_flag));
        AKMDBG("call in %s ,open_flag was set to %d",__func__,atomic_read(&open_flag));  
        wake_up(&open_wq);
        printk(ZTE_TAG "resumed with flag = %d", 
               atomic_read(&reserve_open_flag));
}
...
int akm8962_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        printk(KERN_INFO "akm8962_probe is starting in %s\n", __func__);
        ...
        if (err < 0) {
                printk(KERN_ERR 
                                "AKM8962 akm8962_probe: set power down mode error\n");
                goto exit3;
        }
        #if 0
        /* IRQ */
        ...
        if (!akm->input_dev) {
                err = -ENOMEM;
                printk(KERN_ERR
                       "AKM8962 akm8962_probe: Failed to allocate input device\n");
                goto exit5;
        }else{
                printk(KERN_INFO
                       "AKM8962 akm8962_probe: Success to allocate input device\n");
        }
        ...
        if (err) {
                printk(KERN_ERR
                       "AKM8962 akm8962_probe: Unable to register input device\n");
                goto exit6;
        }else {
                printk(KERN_INFO
                       "AKM8962 akm8962_probe: Register input device success\n");
        }
        
        err = misc_register(&akmd_device);
        if (err) {
                printk(KERN_ERR
                           "AKM8962 akm8962_probe: akmd_device register failed\n");
                goto exit7;
        }else {
                printk(KERN_INFO
                           "AKM8962 akm8962_probe: akmd_device register success\n");
        }
        
        err = misc_register(&akm_aot_device);
        if (err) {
                printk(KERN_ERR
                       "AKM8962 akm8962_probe: akm_aot_device register failed\n");
                goto exit8;
        }else {
                printk(KERN_INFO
                       "AKM8962 akm8962_probe: akm_aot_device register success\n");
        }
