...
static void early_suspend(struct work_struct *work)
{
	...
	list_for_each_entry(pos, &early_suspend_handlers, link) {
		if (pos->suspend != NULL)
		{
			...
			{
				pr_info("NO early_suspend: handlers level = %d \t%pf\n",pos->level,pos->suspend);
				break;
			}
			#endif
			if (debug_mask & DEBUG_SUSPEND)
                        	pr_info("early_suspend: handlers level=%d \t%pf\n", pos->level,pos->suspend);
			pos->suspend(pos);
		}
	}
	mutex_unlock(&early_suspend_lock);
...
static void late_resume(struct work_struct *work)
{
	...
	{ // #ifdef CONFIG_ZTE_PLATFORM
		...
		{
			if (debug_mask & DEBUG_SUSPEND)
                                pr_info("late_resume: handlers level=%d \t%pf\n", pos->level,pos->resume);
			pos->resume(pos);
		}
	}
	if (debug_mask & DEBUG_SUSPEND)
...
void request_suspend_state(suspend_state_t new_state)
{
	unsigned long irqflags;
	int old_sleep;
	int wq_status = -1; // #ifdef CONFIG_ZTE_PLATFORM
