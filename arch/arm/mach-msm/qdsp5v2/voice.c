	...
	case AUDDEV_EVT_FREQ_CHG:
		...
		if (v->dev_state == DEV_READY) {
			...
			MM_ERR("Event not at the proper state =%d\n",
				v->dev_state);
		break;
	case AUDDEV_EVT_AUDIO_LP_START:
	  printk(KERN_ERR "[MyTag]auddev_cb_function: voice_cmd_audio_loopback_start()\n");
		voice_cmd_audio_loopback_start(&voice);
		break;

	case AUDDEV_EVT_AUDIO_LP_END:
				...
				void *evt_buf, u32 len)
{
	...
	case EVENT_NETWORK_RECONFIG:
		...
		complete(&v->complete);
		break;
	case EVENT_AUDIO_LOOPBACK_START:
		printk(KERN_ERR "[MyTag]EVENT_AUDIO_LOOPBACK_START feedback...\n");
		break;
	case EVENT_AUDIO_LOOPBACK_END:
		printk(KERN_ERR "[MyTag]EVENT_AUDIO_LOOPBACK_END feedback...\n");
		break;
	default:
...
static int voice_cmd_audio_loopback_start(struct voice_data *v)
{
	struct voice_header hdr;
	int err;

	hdr.id = CMD_AUDIO_LP_START;
	hdr.data_len = 0;

	printk(KERN_ERR "[MyTag]voice_cmd_audio_loopback_start()");
