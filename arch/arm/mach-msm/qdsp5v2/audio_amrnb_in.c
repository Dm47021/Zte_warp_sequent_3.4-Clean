...
//AUDIO_LZR_CRDB00642553 >.
        if(audio->flushing==1){
            MM_ERR("audio->flushing==1");
            audio->flushing=0;
            mutex_unlock(&audio->read_lock);
            return -1;   //0 byte was read
        }

       if((audio->in_count > 0))
            MM_DBG("audio->in_count > 0)");
       else if(audio->stopped)
            MM_ERR("audio->stopped");
       else if(audio->in_call && audio->running && (audio->voice_state == VOICE_STATE_OFFCALL))
            MM_ERR("voice_state changed to VOICE_STATE_OFFCALL");
       else
            MM_ERR("???");
//AUDIO_LZR_CRDB00642553 .<
		if (rc < 0)
