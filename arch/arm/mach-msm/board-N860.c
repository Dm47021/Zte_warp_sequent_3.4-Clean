/* arch/arm/mach-msm/board-N860
 * Created from the original board-msm7x30.c
 *
 * Credits to CodeAurora and Huawei / Zte
...
static struct msm_camera_sensor_flash_src msm_flash_src_current_driver = {
        ...
        ._fsrc.current_driver_src.driver_channel = &pm8058_ffa_leds_data,
};
...
void msm_snddev_tx_route_config(void)
{
        ...
        rc = gpio_tlmm_config(audio_fluid_icodec_tx_config, GPIO_CFG_ENABLE);
        if (rc) {
...
static ssize_t arthur_virtual_keys_register(struct kobject *kobj,
...
static struct kobj_attribute synaptics_virtual_keys_attr = {
        ...
        .show = &arthur_virtual_keys_register,
};
...
static int synaptics_ts_setup(void)
{
        ...
        if (!virtual_key_properties_kobj || ret)
                pr_err("failed to create arthur board_properties\n");
...
MACHINE_START(ARTHUR, "ZTE ARTHUR")
	.atag_offset = 0x100,
        ...
        .fixup = msm7x30_fixup, // thanks hroark13
MACHINE_END
