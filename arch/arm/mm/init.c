...
void free_initmem(void)
{
	...
	if (!machine_is_integrator() && !machine_is_cintegrator()) {
		printk(KERN_WARNING"free_initmem:  0x%x to  0x%x\n", 
    ...
    printk(KERN_WARNING"start to release SPLASH mem\n");
    //HML add to release splash ram
    printk(KERN_WARNING"finish releaseing SPLASH mem\n");
}
